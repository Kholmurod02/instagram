  ...
  useEffect(() => {
    ...
    const loadDurations = async () => {
      const durations = await Promise.all(
        storyData?.data.stories.map(async (story, index) => {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="max-w-full h-full sm:max-w-full sm:h-full p-0 border-none bg-transparent">
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50">
          ...
          {/* Close button */}
          <button className="absolute top-4 right-4 z-50 text-white" onClick={() => setOpen(false)}>
          ...
          <div className="relative w-full max-w-[400px] sm:max-w-[450px] md:max-w-[500px] h-[70vh] sm:h-[80vh] mx-auto">
            ...
            <div className="absolute top-0 left-0 right-0 flex gap-1 p-2 z-20">
              {storyData?.data?.stories.map((_, index) => {
                const segmentProgress = index < currentIndex ? 100 : index === currentIndex ? progress : 0

                return (
                  <div key={index} className="h-0.5 bg-gray-500/50 flex-1 overflow-hidden rounded-full">
                    <div
            ...
            <div className="absolute top-4 left-0 right-0 flex items-center justify-between px-4 z-20">
              <div className="flex items-center gap-2">
                <Avatar className="w-7 h-7 sm:w-8 sm:h-8 border border-gray-500">
                  ...
                  />
                  <AvatarFallback>{storyData?.data?.userName.substring(0, 2).toUpperCase()}</AvatarFallback>
                </Avatar>
            ...
            <div className="h-full w-full bg-black rounded-md overflow-hidden">
              {storyData?.data?.stories.length > 0 &&
                (isVideoFile(storyData?.data?.stories[currentIndex].fileName) ? (
                  <video
                    ref={(el) => {
