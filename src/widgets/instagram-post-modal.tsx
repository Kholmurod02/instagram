      ...
      `,
			boxShadow:
				adjustments.vignette > 0
					? `inset 0 0 ${adjustments.vignette * 5}px rgba(0,0,0,${
			...
			case 'upload':
				return (
					...
					>
						...
						>
							<title>
								Значок, соответствующий медиафайлам, например изображениям или
								видео
							</title>
							<path
							...
							{selectedImages.length > 0 && (
								<div className='relative w-full h-full'>
									<img
										...
										style={{
											...getAdjustmentStyle(),
											filter: `${getAdjustmentStyle().filter} ${
												filter !== 'Original' ? getFilterStyle(filter) : ''
											}`,
										}}
									/>
										...
										>
											<div className='w-16 h-16 rounded overflow-hidden mb-1'>
												{balloonImageLoaded ? (
													<img
														src={
															filterPreviews[filterName as Filter] ||
															'/placeholder.svg'
														}
														alt={filterName}
														className='object-cover w-full h-full'
														style={{
															filter:
																filterName !== 'Original'
																	? getFilterStyle(filterName as Filter)
																	: 'none',
														}}
													/>
												) : (
