...
const users = [
  { id: 1, name: 'Süveyda Taşkin', username: 'taskinsuveyda' },
  { id: 2, name: 'mohvash_', username: 'mohvash_' },
  { id: 3, name: 'dinara___8', username: 'dinara___8' },
      ...
      <div className="bg-black text-white rounded-lg w-[700px] p-4 shadow-lg ">
            ...
            >
              ...
              <div
                className={`w-5 h-5 border rounded-full flex items-center justify-center ${
                  selectedUsers.includes(user.id) ? 'bg-blue-500 border-blue-500' : 'border-gray-500'
                }`}
              >
                {selectedUsers.includes(user.id) && <div className="w-3 h-3 bg-white rounded-full"></div>}
              </div>
            </div>
          ))}
        </div>
        ...
        <button
          className={`w-full mt-3 py-2 rounded-md font-semibold ${
            selectedUsers.length > 0 ? 'bg-blue-600 text-white' : 'bg-blue-900 text-gray-400 cursor-not-allowed'
          }`}
